...
<div id="content" class="blog box"><?php echo $content_top; ?>

  <div class="box-heading"><h2><?php echo $heading_title; ?></h2></div>
  
  <div class="zee-blog  box-content">
		...
		<div class="blog-meta">
		 ...
		 <div class="blog-content clearfix">
			...
			<div class="zee-video clearfix"><?php echo html_entity_decode($blog['video_code'], ENT_QUOTES, 'UTF-8');?></div>
			<?php } ?>
		 ...
		 <div class="blog-bottom clearfix row-fluid">
				<?php if( !empty($samecategory) ) { ?>
				<div class="span6">
					<h4><?php echo $this->language->get('text_in_same_category');?></h4>
					<ul>
						<?php foreach( $samecategory as $item ) { ?>
						<li><a href="<?php echo $this->url->link('zeeblog/blog',"id=".$item['blog_id']);?>"><?php echo $item['title'];?></a></li>
						<?php } ?>
					</ul>
				</div>
				<?php } ?>
				...
				<?php }else { ?>
					...
					<form action="<?php echo $comment_action;?>" method="post" id="comment-form">
						...
						<div class="recaptcha">
						    ...
						    <br />
						    <img src="index.php?route=zeeblog/blog/captcha" alt="" />
						</div>
						<?php } ?>
