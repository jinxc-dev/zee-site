   ...
   <div id="zeecontentslider<?php echo $id;?>" class="carousel slide zeecontentslider">
		...
		<div class="carousel-inner">
			 <?php foreach ($banners as $i => $banner) {  ?>
				<div class="item <?php if($i==0) {?>active<?php } ?>">
					...
					<div class="zee-caption <?php echo $layer['effect']. " ".$layer['class'];?>" style="top:<?php echo (int)$layer['top']?>px;left:<?php echo (int)$layer['left']?>px">
		...
		<?php if( count($banners)  > 1 ) { ?>	
			...
			<?php foreach ( $banners as $j=>$item )  : ?>
				<li data-target="#zeecontentslider<?php echo $id;?>" data-slide-to="<?php echo $j;?>" class="<?php if($j==0) {?>active<?php } ?>">
