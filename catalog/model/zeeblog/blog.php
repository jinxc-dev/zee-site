...
class ModelZeeblogBlog extends Model {		
	...
	public function getInfo( $id ){
		...
		$query = ' SELECT b.*,bd.title,bd.description,cd.title as category_title,bd.content  FROM '
								. DB_PREFIX . "zeeblog_blog b LEFT JOIN "
								. DB_PREFIX . "zeeblog_blog_description bd ON b.blog_id=bd.blog_id LEFT JOIN "
								. DB_PREFIX . 'zeeblog_category c ON c.category_id=b.category_id  LEFT JOIN ' 
								. DB_PREFIX . 'zeeblog_category_description cd ON (c.category_id=cd.category_id AND cd.language_id='.(int)$this->config->get('config_language_id').')' ;
	...
	public function updateHits( $id ){
		$sql = ' UPDATE '.DB_PREFIX.'zeeblog_blog SET hits=hits+1 WHERE blog_id='.(int)$id;
		$this->db->query( $sql );
	}
	...
	public function getTotal( $data ){
		$sql = ' SELECT count(b.blog_id) as total FROM '
								. DB_PREFIX . "zeeblog_blog b LEFT JOIN "
								. DB_PREFIX . "zeeblog_blog_description bd ON b.blog_id=bd.blog_id  and bd.language_id=".(int)$this->config->get('config_language_id')." LEFT JOIN "
								. DB_PREFIX . 'zeeblog_category c ON c.category_id=b.category_id  LEFT JOIN ' 
								. DB_PREFIX . 'zeeblog_category_description cd ON c.category_id=cd.category_id  and cd.language_id='.(int)$this->config->get('config_language_id') ;
