...
/**
 * class ControllerzeeblogBlog 
 */
	class ControllerzeeblogBlog extends Controller {
		private $mparams = '';
		public function preload(){
			...
			$mparams = $this->config->get( 'zeeblog' );
			$default = $this->model_zeeblog_blog->getDefaultConfig();
		...
		public function index() {  
			...
			$users = $this->model_zeeblog_category->getUsers();
			...
			if ($blog) {
				...
				$this->data['breadcrumbs'][] = array(
					...
					'href'      => $this->url->link('zeeblog/category', 'id=' .  $category_id),      		
					'separator' => $this->language->get('text_separator')
				);	
				$this->data['breadcrumbs'][] = array(
					'text'      => $blog['title'],
					'href'      => $this->url->link('zeeblog/blog', 'id=' .  $blog_id),      		
					'separator' => $this->language->get('text_separator')
				);		
				...
				$blog['category_link'] =  $this->url->link( 'zeeblog/category', "id=".$blog['category_id'] );
				$blog['comment_count'] =  $this->getModel('comment')->countComment( $blog['blog_id'] );
				$blog['link'] =  $this->url->link( 'zeeblog/blog','id='.$blog['blog_id'] );
				...
				$this->data['comment_action'] = $this->url->link( 'zeeblog/blog/comment','id='.$blog['blog_id'] );
				$this->data['blog'] = $blog;
				...
				foreach( $ttags as $tag ){
					$tags[trim($tag)] = $this->url->link( 'zeeblog/blogs','tag='.trim($tag) );
				}
				...
				$this->data['link'] =  $this->url->link( 'zeeblog/blog','id='.$blog['blog_id'] );
				...
				$pagination->url = $this->url->link('zeeblog/blog', 'id=' . $blog['blog_id'] . $url . '&page={page}');
				$data = array(
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/zeeblog/blog.tpl')) {
					$this->template = $this->config->get('config_template') . '/template/zeeblog/blog.tpl';
				} else {
					$this->template = 'default/template/zeeblog/blog.tpl';
				}
