...
/**
 * class ControllerZeeblogSeo inherit core functions of SEO URL class (in common/seo_url.php)
 * Then improving detections to process SEO for zees blog modules. 
 * To use this you should disable Zeestore SEO by comment $controller->addPreAction(new Action('common/seo_url')); in index.php file
 * And Then add This in next line: $controller->addPreAction(new Action('zeeblog/seo'));
 */
	class ControllerZeeblogSeo extends Controller {
		 ...
		 */
		 public function index() {
			...
			if (isset($this->request->get['_route_'])) {
				...
				$blogConfig = $this->config->get('zeeblog');
				...
				if( $this->request->get['_route_'] ==  $seo ){
					$this->request->get['route'] =  'zeeblog/blogs';
				...
				foreach ($parts as $part) {
					...
					if ($query->num_rows) {
						...
						if ($url[0] == 'information_id') {
					 	...
					 	/** BEGIN PROCESSING TO DECORD REQUET SEO URL FOR  PAVO BLOG MODULE **/
						if( count($url) == 2 && ( preg_match( "#zeeblog#", $url[0] ))  ){
		...
		public function rewrite($link) {
			...
			if ( ( preg_match( "#zeeblog#", $data['route'] ))  && isset($data['id']) ) { 
			...
			}  else if( $data['route'] == 'zeeblog/blogs' ){ 
				$blogConfig = $this->config->get('zeeblog');
				$seo = isset($blogConfig['keyword_listing_blogs_page'])?trim($blogConfig['keyword_listing_blogs_page']):"blogs"; 
				$url .= '/'.$seo;
			}
