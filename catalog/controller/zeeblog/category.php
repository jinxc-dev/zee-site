...
/**
 * class ControllerZeeblogCategory 
 */
	class ControllerZeeblogCategory extends Controller {
		...
		public function index() {  
			...
			foreach( $children as $key => $sub ){
				...
				$sub['link']  =  $this->url->link( 'zeeblog/category', 'id=' .  $sub['category_id'] );
		...
		//	echo '<pre>'.print_r( $children,1 ); die;
			...
			if ($category_info) {
				...
				foreach( $blogs as $key => $blog ){
					...
					$blogs[$key]['category_link'] =  $this->url->link( 'zeeblog/category', "id=".$blog['category_id'] );
				...
				$this->data['category_rss'] =  $this->url->link( 'zeeblog/category/rss', "id=".$category_id );
				...
				$pagination->url = $this->url->link('zeeblog/category', 'id=' . $this->request->get['id'] . $url . '&page={page}');
				
				$this->data['pagination'] = $pagination->render();
 
			
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/zeeblog/category.tpl')) {
					$this->template = $this->config->get('config_template') . '/template/zeeblog/category.tpl';
				} else {
					$this->template = 'default/template/zeeblog/category.tpl';
				}
		...
		public function rss(){
			...
			foreach( $blogs as $blog ){
				$link =  str_replace("&amp;","&",$this->url->link( 'zeeblog/blog','id='.$blog['blog_id'] ));
				if( $blog['image'] ){
