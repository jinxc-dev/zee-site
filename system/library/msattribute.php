...
class MsAttribute extends Model {
	...
	public function migrateAttributes() {
		...
		foreach ($this->MsLoader->MsAttribute->getAttributes() as $attribute) {
			...
			$zs_attribute_id = $this->db->getLastId();
			
			// attribute attribute
			$sql = "INSERT INTO " . DB_PREFIX . "ms_attribute_attribute
			SET ms_attribute_id = " . (int)$attribute['attribute_id'] . ",
			zs_attribute_id = " . (int)$zs_attribute_id;
			$this->db->query($sql);
			
			foreach ($attribute['attribute_description'] as $language_id => $value) {
				// oc attribute description
				$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_description SET attribute_id = '" . (int)$zs_attribute_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
			}
		}
	}
	...
	public function updateAttribute($attribute_id, $data) {
		$query = $this->db->query("SELECT zs_attribute_id FROM " . DB_PREFIX . "ms_attribute_attribute WHERE ms_attribute_id = '" . (int)$attribute_id . "' AND zs_attribute_id IS NOT NULL LIMIT 1");
		$zs_attribute_id = $query->row['zs_attribute_id'];
		...
		// oc attribute
		$this->db->query("UPDATE " . DB_PREFIX . "attribute SET sort_order = '" . (int)$data['sort_order'] . "', attribute_group_id = " . (int)$data['attribute_group_id'] . " WHERE attribute_id = " . (int)$zs_attribute_id);

		$this->db->query("DELETE FROM " . DB_PREFIX . "ms_attribute_description WHERE attribute_id = " . (int)$attribute_id);
		$this->db->query("DELETE FROM " . DB_PREFIX . "attribute_description WHERE attribute_id = " . (int)$zs_attribute_id);
	...
	public function deleteAttribute($attribute_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "attribute_description WHERE attribute_id IN (SELECT zs_attribute_id FROM " . DB_PREFIX . "ms_attribute_attribute WHERE ms_attribute_id = $attribute_id AND zs_attribute_id IS NOT NULL)");
	...
	public function getAttribute($attribute_id) {
		$sql = "SELECT *,
						...
						(SELECT attribute_group_id FROM " . DB_PREFIX . "attribute WHERE attribute_id = (SELECT zs_attribute_id FROM  " . DB_PREFIX . "ms_attribute_attribute WHERE ms_attribute_id = " . (int)$attribute_id . ")) as attribute_group_id
