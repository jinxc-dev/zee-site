...
class ModelZeenewsletterDraft extends Model {
	
	public function getTotal($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "zeenewsletter_email` WHERE 1=1";
	...
	public function getList($data = array()) {
		if ($data) {
			$sql = "SELECT `email_id`, `subject`, `date_added`, `to`, `store_id` FROM " . DB_PREFIX . "zeenewsletter_email WHERE 1=1"; 
	...
	public function detail($email_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zeenewsletter_email WHERE email_id = '" . (int)$email_id . "'"); 
		$data = $query->row;

		if ($data) {
			$path = dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'zeedraft_' . $data['email_id'];
	...
	public function delete($email_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "zeenewsletter_email WHERE email_id = '" . (int)$email_id . "'");

		$path = dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'zeedraft_' . $email_id;
		if (file_exists($path) && is_dir($path)) {
			$this->rrmdir($path);
		}
	}
