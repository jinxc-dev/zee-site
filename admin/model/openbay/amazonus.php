...
class ModelOpenbayAmazonus extends Model {
  ...
  <MarketplaceIDs>';
	...
	public function getTotalUnlinkedItemsFromReport() {
		if ($this->openbay->addonLoad('openstock')) {
			$result = $this->db->query("
				SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', zs_sku.product_id, pd.name, zs_sku.sku, zs_sku.var, zs_sku.quantity,
				  (
					...
					WHERE zs_sku.var LIKE CONCAT('%:', pov.product_option_value_id ,':%') OR zs_sku.var LIKE CONCAT(pov.product_option_value_id ,':%')
					  OR zs_sku.var LIKE CONCAT('%:', pov.product_option_value_id) OR zs_sku.var LIKE pov.product_option_value_id
				  ) AS 'combination'
				...
				) AS zs_sku ON alr.sku = zs_sku.sku
				LEFT JOIN " . DB_PREFIX . "amazonus_product_link apl ON (zs_sku.var IS NULL AND zs_sku.product_id = apl.product_id) OR (zs_sku.var IS NOT NULL AND zs_sku.product_id = apl.product_id AND zs_sku.var = apl.var)
				LEFT JOIN " . DB_PREFIX . "product_description pd ON zs_sku.product_id = pd.product_id AND pd.language_id = " . (int)$this->config->get('config_language_id') . "
				WHERE apl.product_id IS NULL
			");
		} else {
			$result = $this->db->query("
				SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', zs_sku.product_id, pd.name, zs_sku.sku, zs_sku.var, zs_sku.quantity, '' AS combination
				FROM " . DB_PREFIX . "amazonus_listing_report alr
				LEFT JOIN (
