	...
	public function getTotalUnlinkedItemsFromReport($marketplace) {
		if ($this->openbay->addonLoad('openstock')) {
			$result = $this->db->query("
				...
				LEFT JOIN " . DB_PREFIX . "amazon_product_link apl ON (zs_sku.var IS NULL AND zs_sku.product_id = apl.product_id) OR (zs_sku.var IS NOT NULL AND zs_sku.product_id = apl.product_id AND zs_sku.var = apl.var)
