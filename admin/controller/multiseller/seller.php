...
class ControllerMultisellerSeller extends ControllerMultisellerBase {
	...
	public function jxPayBalance() {
		...
		$this->data['payment_data'] = array(
			...
			'notify_url' => HTTP_CATALOG . 'index.php?route=payment/zeedownloads-paypal/payoutIPN',
			'custom' => $payment_id
		);
		
		list($this->template) = $this->MsLoader->MsHelper->admLoadTemplate('payment/zeedownloads-paypal');
