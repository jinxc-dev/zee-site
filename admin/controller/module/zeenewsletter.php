...
class ControllerModuleZeenewsletter extends Controller {
	...
	public function initData(){
		...
		$this->load->model('zeenewsletter/newsletter');
		...
		$menus = array();
		$menus["dashboard"] = array("link"=>$this->url->link('module/zeenewsletter', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_dashboard'));
		$menus["create_newsletter"] = array("link"=>$this->url->link('module/zeenewsletter/create_newsletter', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_create_newsletter'));
		$menus["draft"] = array("link"=>$this->url->link('module/zeenewsletter/draft', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_manage_draft_newsletters'));
		$menus["subscribes"] = array("link"=>$this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_manage_subscribes'));
		$menus["templates"] = array("link"=>$this->url->link('module/zeenewsletter/templates', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_templates'));
		$menus["modules"] = array("link"=>$this->url->link('module/zeenewsletter/modules', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_manage_modules'));
		$menus["config"] = array("link"=>$this->url->link('module/zeenewsletter/config', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_global_config'));
		//$menus["information"] = array("link"=>$this->url->link('module/zeenewsletter/information', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_information'));
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('module/zeenewsletter/modules', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		...
		$this->data['action'] = $this->url->link('module/zeenewsletter', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

   		$this->data['yesno'] = array(0=>$this->language->get('text_no'),1=>$this->language->get('text_yes'));

   		$this->document->addStyle('view/stylesheet/zeenewsletter.css');
	}
	/**
	 * set breadcrumb
	 */
	public function setBreadcrumb(){
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/zeenewsletter', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
	}
	public function index() {
		...
		$model = $this->model_zeenewsletter_newsletter;

		$model->installModule();

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('zeenewsletter', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/zeenewsletter', 'token=' . $this->session->data['token'], 'SSL'));
		}


		$this->data['modules'] = array();

		if (isset($this->request->post['zeenewsletter_module'])) {
			$this->data['modules'] = $this->request->post['zeenewsletter_module'];
		} elseif ($this->config->get('zeenewsletter_module')) {
			$this->data['modules'] = $this->config->get('zeenewsletter_module');
		}

		$this->setBreadcrumb();

		$this->data['general'] = $this->config->get('zeenewsletter_config');
		...
		$this->template = 'module/zeenewsletter/panel.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function subsribes(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(isset($post) && $post['action'] == "delete" && !empty($post['selected'])){
				...
				foreach ($selected as $key => $value) {
					$this->model_zeenewsletter_subscribe->delete($value);
				}
			...
			$this->redirect($this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$this->data['action'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$this->data["menu_active"] = "subscribes";

		$subscribe_total = $this->model_zeenewsletter_subscribe->getTotalSubscribers($data);

		$results = $this->model_zeenewsletter_subscribe->getSubscribers($data);
		$stores = $this->model_setting_store->getStores();
		...
		$stores = $tmp;
		$this->data["stores"] = $stores;
		$customer_groups = $this->model_zeenewsletter_subscribe->getCustomerGroups();
		$tmp = array();
		...
		foreach ($results as $result) {
			...
			if($result['action'] == 1){
				...
				'href' => $this->url->link('module/zeenewsletter/unsubsribe', 'token=' . $this->session->data['token'] . '&subscribe_id=' . $result['subscribe_id'] . $url, 'SSL')
				);
			}else{
				$action_name =  $this->language->get('text_no');
				$action[] = array(
				'text' => $this->language->get('text_subscribe'),
				'href' => $this->url->link('module/zeenewsletter/subsribe', 'token=' . $this->session->data['token'] . '&subscribe_id=' . $result['subscribe_id'] . $url, 'SSL')
				);
			}
		...
		$this->data['sort_name'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_email'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . '&sort=email' . $url, 'SSL');
		$this->data['sort_subscribe'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . '&sort=s.action' . $url, 'SSL');
		$this->data['sort_customer_group_id'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . '&sort=customer_group_id' . $url, 'SSL');
		$this->data['sort_store_id'] = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . '&sort=s.store_id' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'module/zeenewsletter/subscribes.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function unsubsribe(){
		...
		if(!empty($subscribe_id)){
			$this->model_zeenewsletter_subscribe->updateAction($subscribe_id, 0);
		}
		$this->redirect($this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'], 'SSL'));
	}
	public function subsribe(){
		...
		if(!empty($subscribe_id)){
			$this->model_zeenewsletter_subscribe->updateAction($subscribe_id, 1);
		}
		$this->redirect($this->url->link('module/zeenewsletter/subsribes', 'token=' . $this->session->data['token'], 'SSL'));
	}
	public function draft(){
		$this->initData();
		$this->setBreadcrumb();
		$this->load->model("zeenewsletter/template");
    $this->load->model("zeenewsletter/draft");
		$this->data["menu_active"] = "draft";
		$this->data['cancel'] = $this->url->link('module/zeenewsletter/templates', 'token=' . $this->session->data['token'], 'SSL');
		$template_id = isset($this->request->get['id'])?$this->request->get['id']:0;
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			  $action = isset($this->request->post["action"])?$this->request->post["action"]:"";
        if($action == "delete"){
            foreach ($this->request->post['selected'] as $draft_id) {
                $this->model_zeenewsletter_draft->delete($draft_id);
            }
            $this->session->data['success'] = $this->language->get('text_success');
        }

        $this->redirect($this->url->link('module/zeenewsletter/draft', 'token=' . $this->session->data['token'], 'SSL'));
      ...
      $total = $this->model_zeenewsletter_draft->getTotal($data);

      $this->data['draft'] = array();

      $results = $this->model_zeenewsletter_draft->getList($data);
      ...
      $this->data['detail'] = $this->url->link('module/zeenewsletter/create_newsletter', 'token=' . $this->session->data['token'] . $url . '&id=', 'SSL');
      $this->data['action'] = $this->url->link('module/zeenewsletter/draft', 'token=' . $this->session->data['token'], 'SSL');
      ...
      $pagination->limit = $this->config->get('config_admin_limit');
      $pagination->text = $this->language->get('text_pagination');
      $pagination->url = $this->url->link('module/zeenewsletter/draft', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      $this->template = 'module/zeenewsletter/draft_newsletter.tpl';
      $this->children = array(
          'common/header',
          'common/footer',
      );
	...
	public function get_template(){
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (isset($this->request->post['store_id'])) {
				...
				if ($store) {
					$url = rtrim($store['url'], '/') . '/index.php?route=module/zeenewsletter/get_template/json';
				} else {
					$url = $store_url . 'index.php?route=module/zeenewsletter/get_template/json';
				}
			} else {
				$url = $store_url . 'index.php?route=module/zeenewsletter/get_template/json';
			}
			...
			$this->redirect($this->url->link('module/zeenewsletter/create_newsletter', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	private function do_request($options) {
		...
		curl_setopt($ch, CURLOPT_USERAGENT, 'ZeeNewsletter for Zeestore');
	...
	public function save_draft(){
      if ($this->request->server['REQUEST_METHOD'] == 'POST') {
          $this->load->model('zeenewsletter/draft');
          $this->load->language('module/zeenewsletter');
          $this->model_zeenewsletter_draft->save($this->request->post);
	...
	public function create_newsletter(){
		if (isset($this->request->get['id']) && $this->request->server['REQUEST_METHOD'] != 'POST') {
			$this->load->model('zeenewsletter/newsletter');
			$this->load->model('zeenewsletter/draft');
			$this->request->post = $this->model_zeenewsletter_draft->detail($this->request->get['id']);
		...
		$this->data['cancel'] = $this->url->link('module/zeenewsletter/templates', 'token=' . $this->session->data['token'], 'SSL');
		$this->data["templates"] = $this->model_zeenewsletter_template->getTemplates();
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			elseif ($action =='check_spam') {
				...
				switch ($this->request->post['to']) {
					case 'subscriber':
						...
						$results = $this->model_zeenewsletter_newsletter->getCustomers($customer_data);
					...
					case 'all':
						...
						$results = $this->model_zeenewsletter_newsletter->getCustomers();
					...
					case 'newsletter':
						...
						$results = $this->model_zeenewsletter_subscribe->getSubscribers($customer_data);
					...
					case 'customer':
						if (isset($this->request->post['customer']) && !empty($this->request->post['customer'])) {
							foreach ($this->request->post['customer'] as $customer_id) {
								$customer_info = $this->model_zeenewsletter_newsletter->getCustomer($customer_id);
						...
						break;
					case 'affiliate_all':
						$results = $this->model_zeenewsletter_newsletter->getAffiliates();
					...
					case 'affiliate':
						if (isset($this->request->post['affiliate']) && !empty($this->request->post['affiliate'])) {
							foreach ($this->request->post['affiliate'] as $affiliate_id) {
								$affiliate_info = $this->model_zeenewsletter_newsletter->getAffiliate($affiliate_id);
					...
					case 'product':
						if (isset($this->request->post['product']) && $this->request->post['product']) {
							$results = $this->model_zeenewsletter_newsletter->getEmailsByProductsOrdered($this->request->post['product']);
						...
						break;
					case 'rewards_all':
						$results = $this->model_zeenewsletter_newsletter->getRecipientsWithRewardPoints();
						...
						break;
					case 'rewards':
						$results = $this->model_zeenewsletter_newsletter->getSubscribedRecipientsWithRewardPoints();
			...
			if ($emails) {
				...
				$data_products = array();
				//get list products
				$this->load->model('zeenewsletter/product');
				$setting = array(
				...
				if($this->request->post['defined']) {
					...
					$products = $this->model_zeenewsletter_product->getProducts();
				...
				if($this->request->post['special']) {
					$specialProducts = $this->model_zeenewsletter_product->getProductSpecials($setting['limit']);
					$data_products['special'] = $this->getItemProducts($specialProducts, $setting);
				}
				
				if($this->request->post['latest']) {
					$latestProducts = $this->model_zeenewsletter_product->getLatestProducts($setting['limit']);
					$data_products['latest'] = $this->getItemProducts($latestProducts, $setting);
				}
				
				if($this->request->post['popular']) {
					$popularProducts = $this->model_zeenewsletter_product->getPopularProducts($setting['limit']);
				...
				$this->model_zeenewsletter_newsletter->send($data);
			} else {
		...
		if (isset($this->error['message'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('module/zeenewsletter/create_newsletter', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		...
		$this->data['action'] = $this->url->link('module/zeenewsletter/create_newsletter', 'token=' . $this->session->data['token'], 'SSL');

    	$this->data['save'] = $this->url->link('module/zeenewsletter/save_draft', 'token=' . $this->session->data['token'], 'SSL');
    ...
    $this->data['affiliates'] = array();
      ...
      if (isset($this->request->post['affiliate']) && is_array($this->request->post['affiliate'])) {
		...
		$this->template = 'module/zeenewsletter/form_newsletter.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function templates(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			switch ($action) {
				...
				case 'copy':
					...
					$check = false;
					if(!empty($templates)){
						$check = $this->model_zeenewsletter_template->copy($templates);
					}
					if($check){
				...
				case 'delete':
					...
					$check = false;
					if(!empty($templates)){
						$check = $this->model_zeenewsletter_template->delete($templates);
					}
					if($check){
		...
		$templates = $this->model_zeenewsletter_template->getTemplates();
		...
		$this->data['insert_link'] = $this->url->link('module/zeenewsletter/template', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action'] = $this->url->link('module/zeenewsletter/templates', 'token=' . $this->session->data['token'], 'SSL');
		$this->template = 'module/zeenewsletter/templates.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function upload() {
		...
		if (!$this->user->hasPermission('modify', 'module/zeenewsletter')) {
		...
		if (!isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_DOWNLOAD .'zeenewsletter/'. $filename . '.' . $ext);
			}
	...
	public function template_from_file(){
		...
		if(!empty($template_file)){
			$file_path = DIR_DOWNLOAD."zeenewsletter/".$template_file;
	...
	public function template(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if($action == "get_template"){
				...
				$check = $this->model_zeenewsletter_template->insertTemplate($this->request->post);
				if($check)
				 	$this->session->data['success'] = $this->language->get('text_zeenewsletter_success');
				else
					$this->session->data['error_warning'] = $this->language->get('text_zeenewsletter_error_warning');

				$this->redirect($this->url->link('module/zeenewsletter/templates', 'token=' . $this->session->data['token'], 'SSL'));
			}

		}
		$template = $this->model_zeenewsletter_template->getTemplate($template_id);
		$this->load->model('localisation/language');
		$languages = $this->model_localisation_language->getLanguages();
		$this->data['languages'] = $languages;
		...
		$this->data['action'] = $this->url->link('module/zeenewsletter/template', 'token=' . $this->session->data['token'], 'SSL');
		$this->template = 'module/zeenewsletter/form_template.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function modules(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//$action = $this->request->post['zeenewsletter_module']['action'];
			$store_id = $this->request->post['zeenewsletter_module']['store_id'];
			$surl = isset($store_id)?'&store_id='.$store_id:'';

			//unset( $this->request->post['zeenewsletter_module']['action'] );
			unset( $this->request->post['zeenewsletter_module']['store_id']);
			unset( $this->request->post['zeenewsletter_module']['stores']);

			$this->model_setting_setting->editSetting('zeenewsletter', $this->request->post, $store_id);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/zeenewsletter/modules', 'token=' . $this->session->data['token'].$surl, 'SSL'));
		}
		...
		if (isset($this->request->post['zeenewsletter_module'])) {
			...
			$setting = $this->model_setting_setting->getSetting("zeenewsletter", $this->data['store_id']);
			$this->data['modules'] = isset($setting['zeenewsletter_module'])?$setting['zeenewsletter_module']:array();
		}
		...
		$this->setBreadcrumb();
    	$this->data['action'] = $this->url->link('module/zeenewsletter/modules', 'token=' . $this->session->data['token'], 'SSL');
		$this->template = 'module/zeenewsletter/frontend_modules.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function config(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('zeenewsletter_config', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/zeenewsletter/config', 'token=' . $this->session->data['token'], 'SSL'));
		}
		$this->data['token'] = $this->session->data['token'];
		...
		if (isset($this->request->post['zeenewsletter_config'])) {
			$this->data['general'] = $this->request->post['zeenewsletter_config'];
		} elseif ($this->config->get('zeenewsletter_config')) {
			$this->data['general'] = $this->config->get('zeenewsletter_config');
		}

		$this->setBreadcrumb();
    $this->data['action'] = $this->url->link('module/zeenewsletter/config', 'token=' . $this->session->data['token'], 'SSL');
		$this->template = 'module/zeenewsletter/config.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
