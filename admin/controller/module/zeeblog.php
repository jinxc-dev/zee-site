...
/**
 * class ControllerModuleZeeblog
 */
class ControllerModuleZeeblog extends Controller {
	...
	public function setTemplate( $tpl ){
		...
		$this->data['manage_category_link'] = $this->url->link('module/zeeblog/category', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['add_category_link'] = $this->url->link('module/zeeblog/category', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['manage_blog_link'] = $this->url->link('module/zeeblog/blogs', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['add_blog_link'] = $this->url->link('module/zeeblog/blog', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['manage_comment_link'] = $this->url->link('module/zeeblog/comments', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['modules_setting_link'] = $this->url->link('module/zeeblog/modules', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['frontend_modules_link'] = $this->url->link('module/zeeblog/frontmodules', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['dashboard_link'] = $this->url->link('module/zeeblog', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['information'] = $this->url->link('module/zeeblog/information', 'token=' . $this->session->data['token'], 'SSL');


		$this->data['token'] = $this->session->data['token'];

		$this->document->addStyle('view/stylesheet/zeeblog.css');
		// $this->data['toolbar'] = $this->
		$this->template =  'module/zeeblog/'.$tpl.".tpl";
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function setBreadcrumb(){
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/zeeblog', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
	}
	...
	public function index(){
		...
		$this->data['mostread'] = $this->model_zeeblog_blog->getMostRead();
		$this->data['newest'] = (array)$this->model_zeeblog_blog->geNewest();
		$this->data['comments'] = (array)$this->model_zeeblog_comment->getNewest();
	...
	public function category(){
		...
		$this->document->addStyle('view/stylesheet/zeeblog.css');
 		$this->document->addScript('view/javascript/zeeblog/jquery.nestable.js');

		$this->load->model('zeeblog/menu');
		// check tables created or not
		$this->model_zeeblog_menu->install();
		$this->load->model('setting/setting');
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  && !empty($this->request->post) ) {

			if( (int)$this->request->post['zeeblog_category']['category_id'] <= 0 ){
				$this->request->post['zeeblog_category']['position'] = '99';
			}
			if(  $this->validateCategory() ) {
				$id = $this->model_zeeblog_menu->editData(  $this->request->post );
				$this->session->data['success'] = $this->language->get('text_success');
				if( $this->request->post['save_mode']=='save-edit'){
					$this->redirect($this->url->link('module/zeeblog/category', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL'));
				}	else {
					$this->redirect($this->url->link('module/zeeblog/category', 'token=' . $this->session->data['token'], 'SSL'));
				}
		...
		$this->data['action'] = $this->url->link('module/zeeblog/category', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionGetTree'] = $this->url->link('module/zeeblog/gettree', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionDel'] = $this->url->link('module/zeeblog/delete', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionGetInfo'] = $this->url->link('module/zeeblog/info', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['updateTree'] = $this->url->link('module/zeeblog/update', 'root=1&token=' . $this->session->data['token'], 'SSL');
		$this->data['cancel'] = $this->url->link('module/zeeblog', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['tree'] = $this->model_zeeblog_menu->getTree(  );
	...
	public function delete(){
		if (!$this->user->hasPermission('modify', 'module/zeeblog')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		if( isset($this->request->get['id']) ){
			$this->load->model('zeeblog/menu');
			$this->model_zeeblog_menu->delete( (int)$this->request->get['id'] );

		}
		$this->redirect($this->url->link('module/zeeblog', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	public function update(){
		...
		if( !empty($data) ){
			...
			$this->load->model('zeeblog/menu');
			$this->model_zeeblog_menu->massUpdate( $data, $root  );
			die("Update done!!!");
		}
	...
	public function info(){
		...
		$menu = $this->model_zeeblog_menu->getInfo( $id );
		$menu = array_merge( $default, $menu );



		$this->data['menu'] = $menu;
		$this->data['menus'] = $this->model_zeeblog_menu->getDropdown(null, $menu['parent_id'] );
		$this->data['thumb'] = $this->model_tool_image->resize($menu['image'], 150, 150);
		$this->data['menu_description'] = array();
		$descriptions  = $this->model_zeeblog_menu->getMenuDescription( $id );
		$this->data['menu_description'] = array();
		...
		$this->template = 'module/zeeblog/category_form.tpl';
	...
	protected function validateCategory() {
		...
		if (isset($this->request->post['zeeblog_category_description'])) {

			$language_id = $this->config->get('config_language_id');

			if( empty($this->request->post['zeeblog_category_description'][$language_id ]['title']) ){
	...
	protected function validateBlog() {
		...
		if (isset($this->request->post['zeeblog_blog_description'])) {

			$language_id = $this->config->get('config_language_id');

			if( empty($this->request->post['zeeblog_blog_description'][$language_id ]['title']) ){
				$this->error['warning'][]=$this->language->get('error_missing_title');
			}
			if( empty($this->request->post['zeeblog_blog_description'][$language_id ]['description']) ){
				$this->error['warning'][]=$this->language->get('error_missing_description');
			}
			if( empty($this->request->post['zeeblog_blog_description'][$language_id ]['content']) ){
	...
	protected function preProcess(){
		...
		$this->load->model('zeeblog/install');
	...
	public function blog(){
		...
		if (isset($this->request->post) && isset($this->request->post['zeeblog_blog']) ) {
			if( $this->validateBlog() ) {
				$id = $this->model_zeeblog_blog->saveData( $this->request->post );
				$action = $this->request->post['action_mode'];
				if( $action == 'save-edit' ){
					$this->redirect($this->url->link('module/zeeblog/blog', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL'));
				}elseif( $action == 'save-new' ){
					$this->redirect($this->url->link('module/zeeblog/blog', 'token=' . $this->session->data['token'], 'SSL'));
				}else {
					$this->redirect($this->url->link('module/zeeblog/blogs', 'token=' . $this->session->data['token'], 'SSL'));
				}
			}
		}
		$this->data['text_image_manager'] = $this->language->get('text_image_manager');
		...
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
		if( isset($this->request->get['id']) ){
			$data = $this->model_zeeblog_blog->getBlog( (int)$this->request->get['id'] );
		...
		$this->data['action'] = $this->url->link('module/zeeblog/blog', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action_delete'] = $this->url->link('module/zeeblog/deleteblog', 'id='.$blog['blog_id'].'&token=' . $this->session->data['token'], 'SSL');
		$this->data['menus'] = $this->model_zeeblog_menu->getDropdown(null, $blog['category_id'], 'zeeblog_blog[category_id]' );
		$this->data['yesno'] = array( 0 => $this->language->get('no'), 1=>$this->language->get('yes') );
		$this->data['blog'] = $blog;
		$this->data['zeeblog_blog_descriptions'] = $blog_descriptions;
	...
	public function blogs(){
		...
		$this->data['menus'] = $this->model_zeeblog_menu->getDropdown(null, $filter['category_id'], 'filter[category_id]' );

		$this->data['filter'] = $filter;
		$this->data['create_blog_link'] = $this->url->link('module/zeeblog/blog', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['edit_link'] = $this->url->link('module/zeeblog/blog', 'id=%s&token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['action'] = $this->url->link('module/zeeblog/blogs', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action_reset'] = $this->url->link('module/zeeblog/blogs', 'reset=true&token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['blogs'] = $this->model_zeeblog_blog->getList( $data, $filter );
		$total =  $this->model_zeeblog_blog->getTotal( $data, $filter );
		$pagination = new Pagination();
		$pagination->total = $total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/zeeblog/blogs', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	public function deleteblog(){
		...
		if( isset($this->request->get['id']) && $id=$this->request->get['id'] ){
			$this->loadModel( 'blog' );
			$this->model_zeeblog_blog->delete( $id );
		}
		$this->redirect( $this->url->link('module/zeeblog/blogs', 'token=' . $this->session->data['token'], 'SSL') );
	}

	public function getModel( $model ){
		return $this->{"model_zeeblog_".$model};
	}

	public function loadModel( $model ){
		$this->load->model( 'zeeblog/'.$model );
	}
	...
	public function comments(){
		...
		$this->data['action'] = $this->url->link('module/zeeblog/comments', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function comment(){
		if( isset($this->request->get['id']) && $id=$this->request->get['id'] ){
			...
			if( isset($this->request->post['action_mode']) && isset($this->request->post['zeeblog_comment']) ){
				$this->getModel('comment')->saveComment( $this->request->post['zeeblog_comment'] );
				if( $this->request->post['action_mode']=='save-edit' ){
					$this->redirect( $this->url->link('module/zeeblog/comment', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL' ) );
				}else {
					$this->redirect( $this->url->link('module/zeeblog/comments', 'token=' . $this->session->data['token'], 'SSL' ) );
				}
			}
			$this->setBreadcrumb();
			$this->document->setTitle( $this->language->get('blogs_page_heading_title') );
			$this->data['heading_title'] =  $this->language->get('blogs_page_heading_title');
			$this->data['action'] = $this->url->link('module/zeeblog/comment', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL');
			$this->data['action_delete'] = $this->url->link('module/zeeblog/deletecomment', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL');
			...
			$tmp = $this->model_zeeblog_comment->getComment( $id );
			if( $tmp ){
	...
	private function getModuleSetting(){
		...
		$general_setting = $this->config->get('zeeblog');
	...
	public function modules(){
		$this->preProcess();
		$this->load->model('setting/setting');
		if( isset($this->request->post['zeeblog']) ){
			$data = array();

			$data['zeeblog'] = $this->request->post['zeeblog'];
			$this->model_setting_setting->editSetting('zeeblog', $data );

			$this->redirect( $this->url->link('module/zeeblog/modules', 'token=' . $this->session->data['token'], 'SSL') );
		...
		$this->data['token'] = $this->session->data['token'];
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
		$this->data['action'] = $this->url->link('module/zeeblog/modules', 'token=' . $this->session->data['token'], 'SSL');
	...
	protected function moduleValidate($module = "") {
		if ($module == "zeebloglatest_module" && isset($this->request->post[$module])) {
	...
	public function frontmodules(){
		$this->preProcess();
		$this->load->model('setting/setting');
		if( isset($this->request->post['zeeblog_frontmodules']) ){
			$data = array();

			$data['zeeblog_frontmodules'] = $this->request->post['zeeblog_frontmodules'];
			/*Updated config for modules*/
			$post = $this->request->post;
			$data = $this->request->post['zeeblog_frontmodules'];
			$this->request->post= array();
			$this->request->post['zeeblog_frontmodules'] = $data;
			$this->model_setting_setting->editSetting('zeeblog_frontmodules', $this->request->post);
			...
			$this->redirect( $this->url->link('module/zeeblog/frontmodules', 'token=' . $this->session->data['token'], 'SSL') );
		...
		$this->data['token'] = $this->session->data['token'];
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
		$this->data['action'] = $this->url->link('module/zeeblog/frontmodules', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['tabs'] = $tabs;
		$this->data['options'] =  $this->model_zeeblog_menu->genOption( 1 ,1, '' );
                  ...
                  foreach ($modules as $index => $module) {
                      switch ($key) {
                          case 'zeeblogcategory':
                              ...
                              $module['menus'] = $this->model_zeeblog_menu->getDropdown(null, $module['category_id'], $key.'_module['.($index).'][category_id]' );
                              break;
