<?php
class ControllerModuleZeecontentslider extends Controller {
	private $error = array(); 
	 
	public function index() {   
		$this->language->load('module/zeecontentslider');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->request->post['zeecontentslider_module'][0]['banner_image'] = 	$this->request->post['banner_image'];
			unset($this->request->post['banner_image']);
			
	
			$this->model_setting_setting->editSetting('zeecontentslider', $this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->redirect($this->url->link('module/zeecontentslider', 'token=' . $this->session->data['token'], 'SSL'));
		}
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/zeecontentslider', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/zeecontentslider', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['zeecontentslider_module'])) {
			$this->data['modules'] = $this->request->post['zeecontentslider_module'];
		} elseif ($this->config->get('zeecontentslider_module')) { 
			$this->data['modules'] = $this->config->get('zeecontentslider_module');
		}	
		...
		$this->document->addStyle('view/stylesheet/zeecontentslider.css');
		$this->template = 'module/zeecontentslider.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function typo(){
		...
		if (isset($this->request->get['field'])) {
	 	...
	 	$typoFile = 	HTTP_CATALOG."catalog/view/theme/default/stylesheet/zeecontentslider.css";	
		if( file_exists( DIR_CATALOG ."view/theme/". $this->config->get('config_template')."/stylesheet/zeecontentslider.css" ) ){
			$typoFile = 	HTTP_CATALOG."catalog/view/theme/". $this->config->get('config_template')."/stylesheet/zeecontentslider.css";	
		}
		$content = file_get_contents(  $typoFile );

		$this->data['typoFile'] = $typoFile; 
		$data = preg_match_all("#\.zee-caption\.([\w\-]+)\s*{\s*#", $content, $matches);
		...
		$this->template = 'module/zeecontentslider_typo.tpl';
		$this->response->setOutput($this->render());
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/zeecontentslider')) {
