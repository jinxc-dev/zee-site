...
//
// Command line tool for installing zeestore
...
//                               --agree_tnc yes \
//                               --http_server http://localhost/zeestore
//
...
function check_requirements() {
	$error = null;
	if (phpversion() < '5.0') {
		$error = 'Warning: You need to use PHP5 or above for ZeeStore to work!';
	}
	...
	if (ini_get('session.auto_start')) {
		$error = 'Warning: ZeeStore will not work with session.auto_start enabled!';
	}

	if (!extension_loaded('mysql')) {
		$error = 'Warning: MySQL extension needs to be loaded for ZeeStore to work!';
	}

	if (!extension_loaded('gd')) {
		$error = 'Warning: GD extension needs to be loaded for ZeeStore to work!';
	}

	if (!extension_loaded('curl')) {
		$error = 'Warning: CURL extension needs to be loaded for ZeeStore to work!';
	}

	if (!function_exists('mcrypt_encrypt')) {
		$error = 'Warning: mCrypt extension needs to be loaded for ZeeStore to work!';
	}

	if (!extension_loaded('zlib')) {
		$error = 'Warning: ZLIB extension needs to be loaded for ZeeStore to work!';
	}

	if (!is_writable(DIR_OPENCART . 'config.php')) {
		$error = 'Warning: config.php needs to be writable for ZeeStore to be installed!';
	}

	if (!is_writable(DIR_OPENCART . 'admin/config.php')) {
		$error = 'Warning: admin/config.php needs to be writable for ZeeStore to be installed!';
	}

	if (!is_writable(DIR_SYSTEM . 'cache')) {
		$error = 'Warning: Cache directory needs to be writable for ZeeStore to work!';
	}

	if (!is_writable(DIR_SYSTEM . 'logs')) {
		$error = 'Warning: Logs directory needs to be writable for ZeeStore to work!';
	}

	if (!is_writable(DIR_OPENCART . 'image')) {
		$error = 'Warning: Image directory needs to be writable for ZeeStore to work!';
	}

	if (!is_writable(DIR_OPENCART . 'image/cache')) {
		$error = 'Warning: Image cache directory needs to be writable for ZeeStore to work!';
	}

	if (!is_writable(DIR_OPENCART . 'image/data')) {
		$error = 'Warning: Image data directory needs to be writable for ZeeStore to work!';
	}

	if (!is_writable(DIR_OPENCART . 'download')) {
		$error = 'Warning: Download directory needs to be writable for ZeeStore to work!';
	}
...
case "install":
	try {
		...
		install($options);
		echo "SUCCESS! Zeestore successfully installed on your server\n";
