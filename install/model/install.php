...
class ModelInstall extends Model {
	public function database($data) {
		...
		$file = DIR_APPLICATION . 'zeestore.sql';
			...
			foreach($lines as $line) {
				if ($line && (substr($line, 0, 2) != '--') && (substr($line, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$sql = str_replace("DROP TABLE IF EXISTS `zs_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("CREATE TABLE `zs_", "CREATE TABLE `" . $data['db_prefix'], $sql);
						$sql = str_replace("INSERT INTO `zs_", "INSERT INTO `" . $data['db_prefix'], $sql);
